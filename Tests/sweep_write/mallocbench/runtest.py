...
pages = [1,2,4,16,64,256,1024,4192,16768,128000]
repeat = 1
...
def testTrace(page):
	...
	for i in xrange(numFs):
		...
		p = Popen('../../../Tests/Tests/FtraceScripts/traceprocess.sh $PWD/mallocbench_trace.out $PWD/'+files[i] , shell=True,stdout=PIPE)
		os.waitpid(p.pid, 0)
		output = p.stdout.read().strip()
